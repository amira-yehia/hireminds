...
export default function Register() {
  ...
  const startFaceEnrollment = async () => {
    ...
    try {
      ...
      setCameraError("Camera permission was blocked. Please allow access and try again.");
    }
  };
  ...
  const createFaceDescriptor = (canvas) => {
    ...
    for (let y = 0; y < grid; y += 1) {
      for (let x = 0; x < grid; x += 1) {
        ...
        descriptor.push(Number((total / (image.data.length / 4) / 255).toFixed(5)));
      }
    }
  ...
  const captureFaceSample = () => {
    ...
    setFaceEmbedding(averageDescriptors(nextSamples.map((sample) => sample.descriptor)));
  ...
  const nextPrompt = facePrompts[Math.min(faceSamples.length, requiredFaceSamples - 1)];
          ...
          <>
            <div className="face-enrollment-card">
              <div className="face-camera-frame">
                {cameraStream ? (
                  ...
                  <div className="face-camera-placeholder">
                    ...
                    <span>{enrollmentComplete ? "Enrollment complete" : "Camera is off"}</span>
                  </div>
                )}
              ...
              <div className="face-enrollment-copy">
                <strong>{enrollmentComplete ? "Face ID sample ready" : nextPrompt}</strong>
                <p>
              ...
              <div className="face-enrollment-actions">
                ...
                <button type="button" className="secondary" onClick={resetFaceEnrollment}>
                  Reset
                </button>
              </div>
            </div>
